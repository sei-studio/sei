...
- **Last action:** Quick task 260516-x62 — ship 3 default personas (Sui / Mochineko / Clawd) with pre-baked expanded prompts so first launch burns no API call; defaults seeded via persistent `defaults-seeded.json` tracker so user deletions stick; CharacterCard DEFAULT chip keyed off `is_default` instead of hardcoded id; `listCharacters` self-heals legacy `persona_prompt`/`description` files; bundled `personaExpansion.ts` validation relaxation (em-dash tolerance + per-section regex + diagnostic logs) that fixes the chars.save IPC blocker. User's 3 legacy on-disk char files removed as one-off cleanup.
- **Next action:** `/gsd-discuss-phase 6` — Scavenging redesign (promoted from 999.1, three coupled subsystems).

---
*Last updated: 2026-05-17 — Completed quick task 260516-x62: ship 3 default personas + auto-purge legacy character files.*
