...
- **Continuity + analytics** follow the contracts below: `REMEMBER_TOOL` honored
  ...
  chat messages the companion actually said.
- **Tool-array policy:** ONE array for every tick kind (chess-style). Ticks land
  well inside the cache TTL, so per-tick-kind arrays would invalidate the prefix
  almost every turn for nothing.
