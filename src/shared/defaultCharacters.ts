 ...
 *   - `runUuidRenameMigration` (main/migration.ts) to relocate a legacy on-disk
 *     slug-named copy to its UUID,
 *   - `runDefaultsToWorldMigration` (main/migration.ts) to convert any leftover
 ...
 *     else, and
 *   - the renderer's fresh-onboarding Sui seed (260728 — OnboardingScreen adds
 *     Sui to the new user's party via chars:add-to-library), which is why this
 *     module lives in src/shared rather than src/main.
 */
 ...
 * (main/migration.ts) stamps this owner onto any pre-existing local `is_default`
