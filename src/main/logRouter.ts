...
/**
 * ITEM 2 (quick/260523-t8d): the system-message classification path is purely
 * regex-based and does NOT apply any toUpperCase / toLocaleUpperCase /
 * toLowerCase coercion to the line text. Brain-emitted system messages
 * (e.g. "[CONSOLE] [sei] Sei online.") flow through with their original
 * casing preserved. The [CONSOLE] literal token used by the brain's system
 * messages is matched by the existing TAG_RE (any bracketed tag), so it
 * classifies as a normal tagged log entry without special handling.
 */
