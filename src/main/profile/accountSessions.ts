 ...
 *
 * Order:
 *   1. The game surfaces, the call, and any chat turn still waiting on the
 *      model (aborted: its reply would be billed to the incoming account and
 *      written wherever the scope points when it lands). Their synchronous
 ...
 *   4. Wait for the closing rows (drainScopedWrites: every end registers its
 *      row writes there), then return; the caller switches scope.
 *
 * The whole thing is bounded by `ceilingMs` (withAccountTeardown): a hung end
 * step must never keep the scope from moving. While it runs
 * isAccountTeardownActive() is true, which defers the rolling-summary fold
 * (see scopeBarrier.ts), refuses new session starts, and tags
 * bot_session_ended.
 */
import {
  ACCOUNT_TEARDOWN_CEILING_MS,
  drainScopedWrites,
  trackScopedWrite,
  withAccountTeardown,
} from './scopeBarrier';
...
export async function endAccountSessions(deps: AccountSessionDeps): Promise<void> {
  ...
  await withAccountTeardown(async () => {
    ...
    const [chess, draw, backseat, chat, chatTurns] = await Promise.all([
      ...
      import('../chat/chatService').catch((err) => (warn('chat turns import', err), null)),
    ]);

    // 1. End the surfaces. Each call starts its end synchronously and
    //    registers its closing rows with the scope barrier. The end promise
    //    itself is registered too (an end that emits its _ended event after
    //    an await must still do so before the scope moves), so step 4's
    //    bounded drain is the only wait: nothing here is awaited unbounded.
    const run = (label: string, fn: () => Promise<void> | void): void => {
      try {
        const p = fn();
        if (p) void trackScopedWrite(p).catch((err) => warn(label, err));
      } catch (err) {
        warn(label, err);
      }
    };
    if (chatTurns) run('chat turns', () => chatTurns.cancelAllInflightTurns());
  ...
  }, deps.ceilingMs ?? ACCOUNT_TEARDOWN_CEILING_MS);
}
