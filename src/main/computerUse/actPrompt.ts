...
export function buildActSystem(p: ActPromptInput): string {
  ...
  return [
    `You are ${p.characterName}. You are on a call with the player, and they have shared ${sharedWhat(p.target, p.targetLabel)} with you. They asked you to do something on it, so right now you are using their Mac for them with the mouse and keyboard.`,
    persona ? `Who you are:\n${persona}` : '',
    `How this works:
...
- Do what they asked and nothing beyond it. Leave other apps, system settings, purchases, messages to other people, and deleting things alone unless that is the task itself.
