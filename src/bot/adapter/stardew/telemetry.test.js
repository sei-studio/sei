...
describe('stardew dashboard telemetry', () => {
  ...
  it('activity lines are lowercase natural language', () => {
    ...
    expect(activityLabel('till', { x: 1, y: 2, width: 4, height: 3 })).toBe('tilling a patch...')
    expect(activityLabel('till', { x: 1, y: 2 })).toBe('tilling the soil...')
    expect(activityLabel('water', { scope: 'farm' })).toBe('watering the farm...')
    expect(activityLabel('harvest', { scope: 'farm' })).toBe('harvesting the farm...')
    expect(activityLabel('ship', {})).toBe('shipping...')
    expect(activityLabel('give', { item: 'Wood' })).toBe('handing over Wood...')
  })
})
