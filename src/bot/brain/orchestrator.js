...
// control), setGoal/clearGoal (goal bookkeeping), quit (session end). Everything
// else that reaches startLongRunner (follow, goto, gather, dig, build, …) is a
// real world action and DOES emit. Kept separate from PERSONALITY_NAMES so
// follow/unfollow still surface a verb even though they're personality-tagged.
const ACTION_VERB_SKIP = new Set(['say', 'remember', 'forget', 'end_loop', 'setGoal', 'clearGoal', 'quit'])
 ...
 */
export function createOrchestrator({ adapter, config, logger = console, sessionState = null, playerStore = null, reenqueue = () => {}, onTerminalError = null, onAuthExpired = null, onSeiChatReply = null, onQuitRequested = null, onAction = null, _anthropicOverride = null }) {
